...
def generate_prompts_for_course(course_id: str, course_name: str, staging_dir: str) -> int:
    ...
    prompt_content = prompt_content.replace("[course_name]", course_name)
...
def generate_master_instructions(course_id: str, course_name: str, staging_dir: str) -> str:
    ...
    content = f"""# Course Generation: {course_name} ({course_id})

You are tasked with generating the full course '{course_name}'.

## Agent Workflow
To ensure accuracy, you must track your own progress and constantly validate your generated files.
1. **Initialize Task Tracker:** Create a `task.md` file in root folder with a checklist of all components to generate (e.g. Chapters, Lessons, Quizzes, Tasks, Side Quests, Projects).
